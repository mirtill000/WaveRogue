...
- `ESP32_SPI_low_power_wakeup` arms ST25R3916/ST25R3916B autonomous Wake-up mode and wakes an ESP32-S3 from deep sleep through the reader `IRQ` pin.
- `ESP32_SPI_low_power_irq_uid` keeps the serial console active for test visibility, waits for the ST25R3916/ST25R3916B Wake-up `IRQ`, then reads one ISO14443A UID and turns the RF field off.
- `ESP32_SPI_power_profile` and `ESP32_SPI_card_current_profile` are measurement helpers for module-level power-current characterization. Their serial commands intentionally use different letters.
